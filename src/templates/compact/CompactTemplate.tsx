...
export const CompactTemplate = forwardRef<HTMLDivElement, TemplateProps>(
  ({ data, scale = 1, isPrinting = false, showPageGuide = true }, ref) => {
    ...
    const {
      fontFamily = 'inter',
      bulletStyle = 'square',
      accentColor = '#2563eb', // Modern Tech Blue
      hiddenSections = [],
    } = settings;
    ...
    const renderSectionTitle = (title: string, isFirstOnPage = false) => {
      ...
      return (
        <div
          ...
          className="section-header w-full pb-0.5 mb-1.5 flex items-center justify-between border-b"
          style={{ paddingTop, borderColor: `${accentColor}40` }}
        >
          <h2
            className={`${fontSizeClasses.sectionTitle} font-bold uppercase tracking-wider`}
            style={{
              color: accentColor,
              margin: 0,
              padding: 0,
          ...
          <span
            className="w-2 h-2 rounded-xs rotate-45"
    ...
    const renderSection = (slot: PageSectionSlot, isFirstOnPage = false) => {
      ...
      switch (sectionKey) {
                  ...
                  >
                    <div className="flex justify-between items-baseline mb-0.5">
                      ...
                      >
                        ...
                        {exp.company && (
                          <>
                            <span className="text-slate-400 mx-1 font-normal">
                              @
                            </span>
                            {exp.companyUrl ? (
                              <a
                      ...
                      <div
                        className={`text-right flex-shrink-0 ml-4 ${fontSizeClasses.subtext} font-medium text-slate-500 whitespace-nowrap`}
                      >
                        ...
                        {exp.highlights
                          ...
                          .map((h, i) => {
                            ...
                            return (
                              <div
                                key={i}
                                className={`${fontSizeClasses.body} ${lineSpacingClasses} text-slate-800 text-justify pl-[10pt] -indent-[10pt]`}
                              >
                                <BulletMarker
                                  style={bulletStyle}
        ...
        case 'references': {
          ...
          const sectionTitle = slot.isContinuation
            ? `${getSectionTitle('references', 'References')} (Cont.)`
              ...
              {page.isFirstPage && (() => {
                ...
                return (
                  <header
                    ...
                    className="resume-header mb-[8pt] pb-2 border-b-2"
                    style={{ borderColor: accentColor }}
                  >
                    <div className="flex justify-between items-center flex-wrap gap-2">
                      <h1
                        className={`${fontSizeClasses.name} font-bold tracking-tight text-slate-900`}
                        style={{
                          lineHeight: 1.1,
                          margin: 0,
                          fontFamily: fontFamilies,
                      ...
                      {contactItems.length > 0 && (
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-slate-600">
                          {contactItems.map((item) => (
                            ...
                            >
                              <ProfileIcon icon={item.icon} size={10} />
                              {item.href ? (
                                <a
                                  href={item.href}
                                  target={
